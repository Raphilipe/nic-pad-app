sap.ui.define([
	...
	"sap/m/FormattedText",
	"sap/m/MessageToast"
], function (BaseController, Device, Controller, Filter, FilterOperator, JSONModel, Fragment, Popup, Dialog, Button, FormattedText, MessageToast) {
	"use strict";
	...
	return BaseController.extend("sap.ui.demo.todo.controller.App", {
		...
		handlePressUserMenu: function (oEvent) {
			...
			if (currentTheme === "sap_horizon") {
				menuItemText = this.getResourceBundle().getText("turnLightsOff");
			} else {
				menuItemText = this.getResourceBundle().getText("turnLightsOn");
			}
			if (!this._userMenu) {
				Fragment.load({
					id: oView.getId(),
					...
					this.getView().byId("menuItemChangeTheme").setText(menuItemText);
					this.getView().byId("menuItemUserName").setText(userName);
					this._userMenu.open(this._bKeyboard, oButton, Popup.Dock.BeginTop, Popup.Dock.BeginBottom, oButton);
				}.bind(this));
			} else {
				this.getView().byId("menuItemChangeTheme").setText(menuItemText);
				this._userMenu.open(this._bKeyboard, oButton, Popup.Dock.BeginTop, Popup.Dock.BeginBottom, oButton);
		...
		onVersionPress: function (oEvent) {
			var that = this;
			$.get("documents/changelog.html", function (data) {
